...
export class EmailsService {
  ...
  async create(createEmailInput: CreateEmailInput): Promise<Email> {
    ...
    return await this.emailsRepository.save(createEmail);
  }
  ...
  async findOne(email: string): Promise<Email> {
    return await this.emailsRepository.findOne({ email });
  }

  async update({ email, name }: UpdateEmailInput): Promise<Email> {
    const updateEmail = await this.emailsRepository.findOne({ email });
    if (!updateEmail) {
      throw new NotFoundException();
    }
    updateEmail.name = name;
    await validateOrReject(updateEmail);
    return await this.emailsRepository.save(updateEmail);
  }

  async remove(email: string): Promise<Email> {
    const deleteEmail = await this.emailsRepository.findOneAndDelete({ email });
